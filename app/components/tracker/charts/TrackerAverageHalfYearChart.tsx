...
export const description = "A line chart showing daily averages";
...
interface TrackerAverageHalfYearChartProps {
  tracker: Tracker;
  entries: Entry[];
}

export function TrackerAverageHalfYearChart({
  tracker,
  entries,
}: TrackerAverageHalfYearChartProps) {
  ...
  dateValues.forEach((value, date) => {
    ...
    const currentMonthData = monthlyData.get(monthKey) || { total: 0, days: 0 };
    monthlyData.set(monthKey, {
      total: currentMonthData.total + value,
      days: currentMonthData.days + 1,
    });
  });
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const averageValue = monthData ? monthData.total / monthData.days : 0;
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Daily Average (last 6 months)</CardTitle>
        <CardDescription>
