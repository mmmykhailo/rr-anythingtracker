...
import { useParams, useLoaderData, useLocation, useSubmit, useNavigation } from "react-router";
import type { ClientLoaderFunctionArgs, ClientActionFunctionArgs } from "react-router";
import { formatDateString } from "~/lib/dates";
import { getTrackerById, createEntry, getTotalValueForDate, getDB, deleteEntryById } from "~/lib/db";
...
export async function clientLoader({ params, request }: ClientLoaderFunctionArgs) {
  ...
  try {
    ...
    return { tracker, currentValue, selectedDate };
  } catch (error) {
  ...
  try {
    if (intent === "addValue") {
      ...
      if (!isNaN(valueToAdd)) {
        ...
        debouncedDataChange.dispatch("entry_added", { trackerId, date, value: valueToAdd });
      }
...
export default function LogEntryPage() {
  ...
  const { tracker, currentValue: loaderCurrentValue, selectedDate: loaderSelectedDate } = useLoaderData<typeof clientLoader>();
