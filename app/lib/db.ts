...
export async function seedInitialData(): Promise<void> {
  ...
  if (trackers.length === 0) {
    ...
    await saveEntry(alcoholTracker.id, formatDate(yesterday), 1);
    await saveEntry(alcoholTracker.id, formatDate(today), 0.2);

    await saveEntry(energyDrinksTracker.id, formatDate(yesterday), 1);
    await saveEntry(energyDrinksTracker.id, formatDate(today), 0.2);

    await saveEntry(coffeeTracker.id, formatDate(yesterday), 0.2);
    await saveEntry(coffeeTracker.id, formatDate(today), 0);
